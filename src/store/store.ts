  ...
  persist(
    ...
    }),
    {
      ...
      migrate: (persistedState: unknown, version: number) => {
        if (version === 0) {
          return { ...persistedState as Record<string, unknown>, productos: PRODUCTOS_INICIALES, materiasPrimas: MATERIAS_PRIMAS_INICIALES, pedidos: [], carrito: [] } as unknown as StoreState;
        }
        if (version === 1) {
          return {
